...
import com.study.mybatis.mapper.DynamicSQLMapper;
...
/**
 * @author RenAshbell
 * @create 2022-05-19-15:02
 */
public class DynamicSQLMapperTest {
    /**
     * 动态SQL(!!为常用):
     * !! 1.if: 根据标签中test属性所对应的表达式决定标签中的内容是否需要拼接到SQL中
     * !! 2.where: 当where标签中有内容时, 会自动生成where关键字, 并且将内容前多余的and 或 or去掉
    ...
    public void testGetEmpByChoose(){
        SqlSession sqlSession = SqlSessionUtils.getSqlSession();
        DynamicSQLMapper mapper = sqlSession.getMapper(DynamicSQLMapper.class);
        List<Emp> list = mapper.getEmpByChoose(new Emp(null, "张三", 23, "男", "61456871@qq.com"));
    ...
    public void testGetEmpByCondition(){
        SqlSession sqlSession = SqlSessionUtils.getSqlSession();
        DynamicSQLMapper mapper = sqlSession.getMapper(DynamicSQLMapper.class);
        List<Emp> list = mapper.getEmpByCondition(new Emp(null, "张三", 23, "男", "61456871@qq.com"));
        System.out.println(list);
    }
}
